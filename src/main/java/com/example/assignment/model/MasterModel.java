...
import com.example.assignment.annocation.Column;
import com.example.assignment.annocation.Table;
import com.example.assignment.ulti.Config.ConfigSql;
import com.example.assignment.ulti.ConnectionHelper;
import com.example.assignment.ulti.PaginationSlave;
...
public class MasterModel<T> {
    ...
    public MasterModel(Class<T> type) {
        this.type = type;
        try {
    ...
    public boolean save(T obj) {
        ...
        if (!objT.isAnnotationPresent(Table.class)) {
            return false;
        }
        Table table = (Table) objT.getDeclaredAnnotation(Table.class);
        if (table.name().length() > 0) {
            tableName = table.name();
        } else {
            tableName = objT.getSimpleName().toLowerCase() + 's';
        }
        try {
